...
TARGET_XSHLDFLAGS= -shared -fPIC --target=$(TARGET) -Wl -install_name $(TARGET_SONAME)
#TARGET_XSHLDFLAGS= -shared -soname=$(TARGET_SONAME) --sysroot=$(SYSROOT)
TARGET_DYNXLDOPTS=
...
$(LUAJIT_SO): $(LJVMCORE_O)
	$(E) "DYNLINK   $@"
	$(Q)$(TARGET_LD) $(TARGET_ASHLDFLAGS) -L$(LIBSROOT) -v -o $@ $(LJVMCORE_DYNO) $(TARGET_ALIBS)
	$(Q)$(TARGET_STRIP) $@

$(LUAJIT_T): $(TARGET_O) $(LUAJIT_O) $(TARGET_DEP)
	$(E) "LINK      $@"
	$(Q)$(TARGET_LD) $(TARGET_ALDFLAGS) -L$(LIBSROOT) -v -o $@ $(LUAJIT_O) $(TARGET_O) $(TARGET_ALIBS)
	$(Q)$(TARGET_STRIP) $@
