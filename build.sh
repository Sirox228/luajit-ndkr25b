...
case "$1" in
    ...
    armeabi-v7a)
        ...
        make -C luajit CC="${CC} -m32" HOST_CC="${CC} -m32" STATIC_CC=${CROSSTC}${CC} DYNAMIC_CC=${CROSSTC}${CC} TARGET_LD=$TOOLCHAIN/bin/ld CFLAGS="-O2 -lm -ldl" HOST_CFLAGS="-O2" TARGET_CFLAGS="${CFLAGS}" CROSS=${CROSSTC} DESTDIR="${DEST}"
        ;;
    arm64-v8a)
        export TOOLCHAIN=$NDK/toolchains/llvm/prebuilt/$HOST_ARCH
